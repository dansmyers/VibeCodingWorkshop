...
<img src="https://static0.polygonimages.com/wordpress/wp-content/uploads/chorus/uploads/chorus_asset/file/22421821/NbKPigG.png" width="600px" />

The original website for *Space Jam* from 1996 still exists. [This is what peak performance looks like](https://www.spacejam.com/1996/).
...
We're going to do this activity **without Claude**, so you can get more hands-on with the basic elements of web design.

## Write and view a basic web page

### Playground

Open a tab and go to [https://seleniumbase.io/w3schools/]. This will open a page where you can enter web code in one pane and view the output in the other.

Delete any text already in the window.

### Creating the page

Let's make a basic web page. Put the following text in your code window (after deleting the code that's already there). I'll tell you more about how this file is put together in the next section, after you've had the
chance to run and display it.

Click the "Run" button at the top of the screen to view the new page.
...
Yikes. That's large. You can add style to the tag to scale the image to a percentage of the display width. Change your `<img>` tag to the following, which sets the image width to 50% of the visible window.
```
<p>
    <img src="https://upload.wikimedia.org/wikipedia/commons/0/0a/The_Great_Wave_off_Kanagawa.jpg" style="width:50%"/>
</p>
```
You can also set an absolute size in pixels, e.g. `style="width:200px"`.
...
Replace the entire `<head>` block with the following:
```
<head>
    <title>This appears at the top of the browser.</title>
...
**Colors** Colors are specified as three values, denoting the red, green, and blue components of the color. Each value is represented using the **hexadecimal** number system, which encodes a byte of data as a value from `00` to `FF`.
...
You can add a setting for the `color` property to change the color of the text.
